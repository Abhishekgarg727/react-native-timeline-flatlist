...
export default class Timeline extends Component {
    ...
    return (
      <View style={[styles.container, this.props.style]}>
        <FlatList
          ...
          keyExtractor={(rowData, rowID) => this.uuidv4()}
          style={[styles.listview, this.props.listViewStyle]}
          renderrowData={this._renderRow}
          {...this.props.options}
        />
      </View>
    );
      ...
      case "single-column-left":
        content = (
          <View style={[styles.rowContainer, this.props.rowContainerStyle]}>
            {this.renderTime({rowData, rowID})}
            {this.renderEvent({rowData, rowID})}
            {this.renderCircle({rowData, rowID})}
          </View>
        );
        break;
  ...
  _renderTime({rowData, rowID}) {
    ...
    switch (this.props.columnFormat) {
      case "single-column-left":
        timeWrapper = {
          alignrowDatas: "flex-end"
        };
        break;
      case "single-column-right":
        timeWrapper = {
          alignrowDatas: "flex-start"
        };
        break;
      case "two-column":
        timeWrapper = {
          flex: 1,
          alignrowDatas: rowID % 2 == 0 ? "flex-end" : "flex-start"
        };
        break;
    }
    return (
